...
public class StringTokenizerTest {
    ...
    @Test
    public void testMultipleIdenticalStrings() throws ParseException {
    ...
    public void testMultipleStrings() throws ParseException {
        List<StringToken> tokens = tokenize(STRINGS[0] + ' ' + STRINGS[1] + "  " + STRINGS[2] + "\t" + STRINGS[3] + " ");
    ...
    public void testQuotedString() throws ParseException {
        ...
        List<StringToken> tokens = tokenize(StringTokenizer.QUOTE_CHAR + joinedStrings + StringTokenizer.QUOTE_CHAR);

        assertStringsEqual(tokens, joinedStrings);
    }

    @Test
    public void testQuotedStringWithoutSpaces() throws ParseException {
        List<StringToken> tokens = tokenize(STRINGS[0] + StringTokenizer.QUOTE_CHAR + STRINGS[1] + StringTokenizer.QUOTE_CHAR);

        assertStringsEqual(tokens, Arrays.copyOf(STRINGS, 2));
    }

    @Test(expected = ParseException.class)
    public void testNotMatchingQuotation() throws ParseException {
    ...
    @Test
    public void testOpenParenthesisBetweenStrings() throws ParseException {
        List<StringToken> tokens = tokenize(STRINGS[0] + "(" + STRINGS[1]);

        Assert.assertNotNull(tokens);
        Assert.assertEquals(3, tokens.size());

        StringToken expectedToken = new StringToken(StringToken.TokenType.OPEN_PARENTHESIS, "(", STRINGS[0].length());
    ...
    @Test
    public void testCloseParenthesisBetweenStrings() throws ParseException {
        List<StringToken> tokens = tokenize(STRINGS[0] + ")" + STRINGS[1]);

        Assert.assertNotNull(tokens);
        Assert.assertEquals(3, tokens.size());

        StringToken expectedToken = new StringToken(StringToken.TokenType.CLOSE_PARENTHESIS, ")", STRINGS[0].length());
        Assert.assertEquals(tokens.get(1), expectedToken);
    }

    @Test
    public void testStringInParenthesis() throws ParseException {
        List<StringToken> tokens = tokenize("(" + STRINGS[0] + ")");

        StringToken expectedTokens[] = {
                new StringToken(StringToken.TokenType.OPEN_PARENTHESIS, "(", 0),
    ...
    private static void assertTokensEqual(List<? extends StringToken> actual, StringToken[] expected) {
    ...
    private static void assertStringsEqual(List<? extends StringToken> actual, List<String> expected) {
        ArrayList<String> actualStrings = actual.stream().map(StringToken::getString).collect(
