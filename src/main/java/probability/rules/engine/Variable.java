...
public class Variable<T> implements Expression, Token {
  ...
  public int compareTo(Object other, ImmutableAttributeHolder bindings) {
    ...
    if (Comparable.class.isAssignableFrom(getType())) {
      ...
      @SuppressWarnings("unchecked") Comparable<Object> comparable =
          (Comparable<Object>) getValue(bindings);
  ...
  @Override public boolean interpret(ImmutableAttributeHolder bindings) {
    ...
    if (!getType().equals(Boolean.class)) {
      ...
      throw new IllegalStateException("only boolean variables can be" + " evaluated directly");
    }
