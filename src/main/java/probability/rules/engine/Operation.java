    ...
    for (Operation op : operations) {
      ...
      if (symbol == null || symbol.isEmpty()) {
        throw new IllegalStateException(
            op.getType().getName() + " does not provide a nonempty symbol");
      }
      if (StringUtils.containsWhitespace(symbol)) {
        throw new IllegalStateException(
            symbol + " for " + op.getType().getName() + " contains invalid symbols");
      }
      if (OPERATIONS_BY_SYMBOL.containsKey(symbol)) {
        throw new IllegalStateException(
            symbol + " for " + op.getType().getName() + " is not unique");
      }
