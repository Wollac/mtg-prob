...
class PlayableRecursion {
  ...
  private final RemainingManaCost _remainingCost;
  ...
  PlayableRecursion(Hand hand, int maxTurn, ManaCost spellCost) {
    ...
    _remainingCost = new RemainingManaCost(spellCost);
    _board = new Board();
  }
  ...
  private boolean recursion(int turn) {
    ...
    for (CardObject landObject : _hand.getNotPlayedLandObjectsUntilTurn(turn)) {
      ...
      final boolean tapped = land.comesIntoPlayTapped(_board);
      ...
      for (Color color : colors) {
        ...
        if (_remainingCost.contains(color)) {

          producesUsableColors = true;
          if (tapped) {
            if (addTappedColorAndTest(color, turn)) {
              return true;
            }
          } else {
            if (addColorAndTest(color, turn)) {
              return true;
            }
          }
        }
    ...
    // as a land can produce at most one mana, it makes never sense to not play a land that could
    // have been used it also makes no sense, if there wont be any land available later on
  ...
  private void removeLandObject(CardObject landObject) {
    ...
    CardObject popObject = _board.pop();
