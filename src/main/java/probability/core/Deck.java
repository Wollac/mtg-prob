...
public class Deck {
  ...
  private static String cardCountsToString(Multiset<Card> cardCounts, CardType type) {
    ...
    for (Card card : CardUtils.sortCardsByName(cardCounts.elementSet())) {
      sb.append(String.format("%2dx " + card + "%n", cardCounts.count(card), card));
    }
  ...
  public void fillWithDummies() {
    if (_cards.size() > Settings.config.numberOfCards()) {
      throw new IllegalStateException(
          "The predefined deck consists of more than " + Settings.config.numberOfCards()
              + " cards");
    }
