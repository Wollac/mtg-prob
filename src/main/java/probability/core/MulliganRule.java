...
public class MulliganRule {
  ...
  public boolean takeMulligan(final Collection<CardObject> startingHand) {
    ...
    Supplier<Integer> landsSupplier =
        Suppliers.memoize(() -> CardUtils.getNumberOfLandObjects(startingHand));
    ...
    @Override protected void read(Reader reader) throws IOException {
      ...
      try {
        ...
        Logger.error(Messages.get()
            .parseFileExceptionWithLineNumber(getFileName(), FILE_TYPE, e.getLocalizedMessage(),
                e.getErrorLine()));
        Logger.debug(e);
      }
    }
