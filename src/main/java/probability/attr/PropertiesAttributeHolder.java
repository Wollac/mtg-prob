...
public class PropertiesAttributeHolder implements AttributeHolder {
   ...
   *
   * <p>For each {@code AttributeKey} the value string of the key corresponding to its name is then
   ...
   *
   * <p>After the entries have been written, the output stream is flushed.
  ...
  public void store(Writer writer, String description) throws IOException {
    ...
    for (AttributeKey<?> key : _attributes) {
      properties.setProperty(key.getName(), _attributeHolder.getAttributeValue(key).toString());
    }
